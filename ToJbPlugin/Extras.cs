...
//DO NOT FORGET TO ADD PARTIAL INTO JAIL CLASS
public partial class JailPlugin
{
    ...
    private HookResult EXTRAOnPlayerPing(EventPlayerPing @event, GameEventInfo info)
    {
        ...
        {
            player.PrintToChat($"You must be an admin or warden to mark");
    ...
    private HookResult EXTRAOnRoundStart(EventRoundStart @event, GameEventInfo info)
    {
        if (Lasers != null)
        {
            foreach (var item in Lasers)
